...
const Detail_club = () => {
  ...
  const [isClubMem, setIsClubMem] = useState(
    userAuth ? userAuth.clubs.includes(clubId) : false
  );
  const [isClubAuth, setIsClubAuth] = useState(false);
  const [isWaitingMem, setIsWaitingMem] = useState(
    userAuth ? userAuth.waitingClubs.includes(clubId) : false
  );
  ...
  useEffect(() => {
    ...
    if (clubInfo && clubInfo._id) {
      setIsClubAuth(
        userAuth && clubInfo ? clubInfo.admin.includes(userAuth._id) : false
      );
    }
  }, [clubInfo, userAuth]);
    ...
    <div>
      ...
      <div className={styles.container}>
          ...
          >
            ...
            <div className="profile-info">
              {isClubAuth && (
                <MemListModal
                  clubId={clubId}
                  userList={clubInfo.members}
                ></MemListModal>
              )}
              {isClubAuth && (
                <ProposeModal
                  clubId={clubId}
                  userList={clubInfo.proposers}
                ></ProposeModal>
              )}
              {isClubAuth && (
                <button
        ...
        <section>
          ...
          <div className={styles.boardGrid} style={{ marginTop: "2%" }}>
            {events
              .filter(
                (event) =>
